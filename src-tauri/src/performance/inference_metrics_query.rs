...
impl InferenceMetricsQuery {
    ...
    pub fn count(metrics: &[InferenceMetrics]) -> usize {
        metrics.len()
    }
}
...
mod tests {
    ...
    fn test_gpu_usage_percent() {
        let metrics: Vec<InferenceMetrics> = (0..10)
            ...
            .collect();
        assert_eq!(InferenceMetricsQuery::gpu_usage_percent(&metrics), 70.0);
    }

    #[test]
    fn test_empty_metrics() {
        let metrics = vec![];
        assert_eq!(InferenceMetricsQuery::avg_tokens_per_second(&metrics), 0.0);
        assert_eq!(InferenceMetricsQuery::avg_inference_time_ms(&metrics), 0.0);
        assert_eq!(InferenceMetricsQuery::gpu_usage_percent(&metrics), 0.0);
    }
}
